...
static AlgorithmFunction g_functions[ImageProcessor::AlgorithmCount] = {
    _gray, //这边用了类似List的方式，把这些方法存到了一个List中，当调用时，外部指定第几个，这里是ImageProcessor::AlgorithmCount的类型
    _binarize,//在传入参数(这里的入参都一致),就实现了调用。例如g_function[0](m,n)==_gray(m,n)
    _negative,//注意，里面的方法要是静态的
    _emboss,
    _sharpen,
    _soften
};
...
public:
    ...
    void run()
    {
        ...
        QCoreApplication::postEvent(m_observer, new ExcutedEvent(this));//类似于放送一个信号出去 接收方重写虚函数enent()处理，（通过自定义事件，然后QApplication::postEvent给主界面，我之前用的是信号槽）
    }
...
void ImageProcessor::Heibai(QString sourceFile, QString destFile)
{
    QUrl url(sourceFile);
